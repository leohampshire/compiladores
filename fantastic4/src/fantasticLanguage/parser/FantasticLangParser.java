...
public class FantasticLangParser extends Parser {
	...
	protected static final DFA[] _decfantasticonToDFA;
	...
	public FantasticLangParser(TokenStream input) {
		super(input);
		_interp = new ParserATNSimulator(this,_ATN,_decfantasticonToDFA,_sharedContextCache);
	}
	...
	public final CmdselecaoContext cmdselecao() throws RecognitionException {
		...
		try {
			...
			setState(92);
			match(ID);
			 _exprDecfantasticon = _input.LT(-1).getText();
			setState(94);
			match(OPREL);
			 _exprDecfantasticon += _input.LT(-1).getText();
			setState(96);
	...
	static {
		_decfantasticonToDFA = new DFA[_ATN.getNumberOfDecfantasticons()];
		for (int i = 0; i < _ATN.getNumberOfDecfantasticons(); i++) {
			_decfantasticonToDFA[i] = new DFA(_ATN.getDecfantasticonState(i), i);
		}
	}
}